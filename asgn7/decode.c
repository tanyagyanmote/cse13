...
int main(int argc, char **argv) {
  ...
  // readinng the dumped tree from inputfile with the array which is tree_size
  // long
  ...
  Node *root = huff_tree;
  // making sure the symbol is less than file size, checking if all symbols are
  // read
  while (symbol < m.file_size) {
    ...
    if (read_bit(inputfile, &bit)) {
      ...
      if (root->left == NULL) {
        if (root->right == NULL) {
          // if the root we are at is a leaf node then write the symbol to the
          // outputfile
