...
BitVector *bv_create(uint32_t length) {
  // allocating memeory for the bit vector
  BitVector *bv = (BitVector *)malloc(sizeof(BitVector));
  // null check
  if (bv != NULL) {
    // taking the amount of bits and figuring out how many bits there are
    bv->length = length;
    if (length % 64 != 0) {
      length = (length / 64) + 1;
    } else {
    ...
    // if its null then free the bit vector and return null
    if (bv->vector == NULL) {
      free(bv);
      return NULL;
    }
